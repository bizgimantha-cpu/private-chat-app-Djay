...
(async () => {
    ...
    if (count && count.n === 0 && ADMIN_PASSWORD !== "CHANGE_ME_NOW") console.log("Admin credentials are set by environment variables.");
})();
...
app.post("/api/users", auth, admin, async (req, res) => {
    ...
    try { 
        ...
        await run("INSERT INTO users VALUES(?,?,?,?,?,?)", [id, username, await bcrypt.hash(password, 12), displayName || username, 0, Date.now()]); 
        res.json({ ok: true }) 
    } catch (e) { 
...
app.post("/api/invite/register", async (req, res) => {
    ...
    try { 
        await run("INSERT INTO users VALUES(?,?,?,?,?,?)", [crypto.randomUUID(), username, await bcrypt.hash(password, 12), displayName || username, 0, Date.now()]); 
        res.json({ ok: true }) 
    } catch (e) { 
        res.status(409).json({ error: "Username already exists" }) 
    }
